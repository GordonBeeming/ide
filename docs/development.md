...
- Workspace content search runs in Rust, walks the workspace by layer so shallow matches spend the result cap first, skips generated folders and binary-looking files, and applies the Settings-backed result/file-size caps before scanning. Search responses report when the result cap truncated matches so the UI can link back to Search settings instead of implying the result set is complete. File opens use the Settings-backed editable file size cap.
