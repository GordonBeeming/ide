...
describe("hosted Tauri API transport", () => {
  ...
  it("passes explicit quick-open limits through hosted indexed file search", async () => {
    ...
    await searchIndexedFiles("app", 20, false, false, false);
    ...
    expect(fetchMock.mock.calls[2][0]).toBe(
      "/api/file-search?query=app&limit=20&showGitignoredFiles=false",
    );
  });
  ...
  it("passes dotfile visibility through hosted file listing requests", async () => {
    ...
    await listFiles(false, false, undefined, false);
    ...
    expect(fetchMock.mock.calls[5][0]).toBe("/api/files?showGitignoredFiles=false");
  });
  ...
  it("passes visibility settings through hosted directory listing requests", async () => {
    ...
    await listDirectory("src folder", false, false, false);
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "/api/directory?path=src+folder&showGitignoredFiles=false",
      expect.objectContaining({ method: "GET" }),
    );
  });
